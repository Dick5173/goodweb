...
<script>
    ...
    export default{
        ...
        computed:{
          orderTotal () {
            return parseInt(this.subTotal) - parseInt(this.shipping) - parseInt(this.discount) - parseInt(this.tax)
          }
        ...
        methods: {
          init(){
            ...
            .then(function(response){
              ...
              let money = 0
              if(thisT.cartList.length){
                thisT.cartList.forEach((item) => {
                  if(item.checked === '1') {
                    money += item.prodcutPrice * item.productNum
                  }
                })
                thisT.subTotal = money
              }
            })
          },
          payMent () {
            ...
            axios.post('/api/users/payMent',{
              addressId: thisT.addressId,
              orderTotal: thisT.orderTotal
            })
              .then(function (response) {
