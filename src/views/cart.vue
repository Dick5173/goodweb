                ...
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;" class="checkbox-btn item-check-btn"
                     v-bind:class="{'check':item.checked=='1'}" @click="addCart('checked',item)" >
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="addCart('minus',item)">-</a>
                        <span class="select-ipt">{{ item.productNum }}</span>
                        <a class="input-add"  @click="addCart('add',item)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div class="cart-foot-wrap">
          <div class="cart-foot-inner">
            ...
            <div class="cart-foot-r">
              ...
              <div class="btn-wrap">
                <a class="btn btn--red" v-bind:class="{ 'btn--dis': checkOutCount === 0}"
        ...
        computed:{
          ...
          totalPrice () {
            ...
            if(this.cartList.length){
              this.cartList.forEach((item) => {
                if(item.checked === '1') {
                  money += item.prodcutPrice * item.productNum
                }
              })
              return money
            }
        ...
        methods: {
          ...
          delCart(){
            ...
            axios.post('/api/users/cart/del',{
              ...
              if(res.status === '0'){
                // thisT.init()
                for(let [key,value] of thisT.cartList.entries()){
                  if(value.productId === thisT.delItem.productId){
                    thisT.cartList.splice(key,1)
                  }
                }
              }
            })
